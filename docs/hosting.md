...
> [!warning]
> Cloudflare Pages will shallow `git` clones so if you rely on `git` for timestamps, it is recommended you change build command to `git fetch --unshallow && npx quartz build`.
